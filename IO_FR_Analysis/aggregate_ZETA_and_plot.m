...
for k = 1:numel(fileList)
    ...
    % =====================================================================
    % NEW: attach dynamic ZETA + IFR/PSTH metrics from per-case MAT files
    % =====================================================================
    ...
    ZETA_invSign_Latency    = nan(height(T),1);
    IFR_peakLatency         = nan(height(T),1);
    IFR_peakOnset_Latency   = nan(height(T),1);

    % ---- NEW: corresponding values from sZETA.vecLatencyVals (optional) ----
    ZETA_peakLatencyVal      = nan(height(T),1);
    ZETA_invSign_LatencyVal  = nan(height(T),1);
    IFR_peakLatencyVal       = nan(height(T),1);
    IFR_peakOnset_LatencyVal = nan(height(T),1);
    ...
    if hasIFRMat
        ...
        IFRsum = S2.IFR_PSTH_Summary; % kept only for metadata if needed
        all_IFR = S2.all_IFR;
    ...
    for r = 1:height(T)
        ...
        % --- NEW: vecLatencies + vecLatencyVals extraction ---
        if hasZetaMat && isKey(zetaMap, key)
            idxZ = zetaMap(key);
            sZ   = all_sZETA{idxZ};

            if isfield(sZ,'vecLatencies') && ~isempty(sZ.vecLatencies)
                vL = double(sZ.vecLatencies(:));
                % Expected order:
                % 1) ZETA peak latency
                % 2) inverse sign latency
                % 3) IFR peak time
                % 4) IFR peak onset (half-height crossing)
                if numel(vL) >= 1, ZETA_peakLatency(r)      = vL(1); end
                if numel(vL) >= 2, ZETA_invSign_Latency(r)  = vL(2); end
                if numel(vL) >= 3, IFR_peakLatency(r)       = vL(3); end
                if numel(vL) >= 4, IFR_peakOnset_Latency(r) = vL(4); end
            end

            if isfield(sZ,'vecLatencyVals') && ~isempty(sZ.vecLatencyVals)
                vV = double(sZ.vecLatencyVals(:));
                if numel(vV) >= 1, ZETA_peakLatencyVal(r)      = vV(1); end
                if numel(vV) >= 2, ZETA_invSign_LatencyVal(r)  = vV(2); end
                if numel(vV) >= 3, IFR_peakLatencyVal(r)       = vV(3); end
                if numel(vV) >= 4, IFR_peakOnset_LatencyVal(r) = vV(4); end
            end
        end
    ...
    % 1) ZETA_peakLatency, 2) ZETA_invSign_Latency
    % 3) IFR_peakTime, 4) IFR_peakOnset_Latency
    T.ZETA_peakLatency         = ZETA_peakLatency;
    T.ZETA_invSign_Latency     = ZETA_invSign_Latency;
    T.IFR_peakLatency          = IFR_peakLatency;
    T.IFR_peakOnset_Latency    = IFR_peakOnset_Latency;

    T.ZETA_peakLatencyVal      = ZETA_peakLatencyVal;
...
title(sprintf('ZETA z-scores | All conditions (MoveType × Depth)  (N=%d hemispheres)', numel(uniqSubsAll)));
...
for dd = 1:numel(depthKeys)
    ...
    % Use depth color for subplot annotation
    if isKey(depthColorMap, dzKey)
        dc = depthColorMap(dzKey);
    else
        dc = fallbackCol;
    end
    ...
    % tt = title(ax, sprintf('All MoveTypes | %s', depthNames{dd}));
    % set(tt,'Color',dc);
end
...
title(tlo, sprintf('ZETA z-scores | All Movement Types by STN depth '));
...
%% 5) ZETA scatter by depth (rows) with x = MoveType (REST, HAND OC, HAND PS, ARM EF)
