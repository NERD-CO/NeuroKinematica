function [ outthresh , outKclu ] = threshEstimate_v2b( sampData , spkData,  spkFS , posnegFlag )
...
while crit
    ...
    if posnegFlag
        ...
        noise = (mean(double(sampData)) + round(std(double(sampData)) * 20));
    else
        thresh = (mean(double(sampData)) - round(std(double(sampData)) * startThresh));
        noise = (mean(double(sampData)) - round(std(double(sampData)) * 20));
    end
    ...
    [ waveData ] = extractWaveforms_Clz_v01b(spkData, thresh, noise, minDist, spkFS);
