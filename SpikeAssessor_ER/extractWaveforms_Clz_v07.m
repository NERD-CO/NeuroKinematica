...
totLen = length(1 - (sampLen*0.75):1 + (round(sampLen*1)-1));
...
for ai = 1:length(waves)
    ...
    locP = waves(ai);
    if nOp == -1
        sampPeriod = locP - (round(sampLen*1)-1):locP + (sampLen*0.75);
    elseif nOp == 1
        sampPeriod = locP - (sampLen*0.75):locP + (round(sampLen*1)-1);
    end
