...
% Determine minimum dataset size for amplitudes, widths, peakDists
minSize_amplitudes = min([length(conditionsData.OffOff.amplitudes), length(conditionsData.OffOn.amplitudes), length(conditionsData.OnOff.amplitudes), length(conditionsData.OnOn.amplitudes)]);
minSize_widths = min([length(conditionsData.OffOff.widths), length(conditionsData.OffOn.widths), length(conditionsData.OnOff.widths), length(conditionsData.OnOn.widths)]);
minSize_peakDists = min([length(conditionsData.OffOff.peakDists), length(conditionsData.OffOn.peakDists), length(conditionsData.OnOff.peakDists), length(conditionsData.OnOn.peakDists)]);


% % Trim and combine amplitude data
% trimmed_amplitudes = arrayfun(@(x) rand_trim_dataset(conditionsData.(char(x)).amplitudes, minSize_amplitudes), {'OffOff', 'OffOn', 'OnOff', 'OnOn'}, 'UniformOutput', false);
% all_amplitudes = vertcat(trimmed_amplitudes{:});
%
% % Trim and combine width data
% trimmed_widths = arrayfun(@(x) rand_trim_dataset(conditionsData.(char(x)).widths, minSize_widths), {'OffOff', 'OffOn', 'OnOff', 'OnOn'}, 'UniformOutput', false);
% all_widths = vertcat(trimmed_widths{:});
%
% % Trim and combine peakDist data
% trimmed_peakDists = arrayfun(@(x) rand_trim_dataset(conditionsData.(char(x)).peakDists, minSize_peakDists), {'OffOff', 'OffOn', 'OnOff', 'OnOn'}, 'UniformOutput', false);
% all_peakDists = vertcat(trimmed_peakDists{:});
%
%
% % Create corresponding group labels
% group_labels_amplitudes = arrayfun(@(x) repmat({x}, size(trimmed_amplitudes{strcmp({'OffOff', 'OffOn', 'OnOff', 'OnOn'}, x)})), {'OffOff', 'OffOn', 'OnOff', 'OnOn'}, 'UniformOutput', false);
% group_labels_amplitudes = vertcat(group_labels_amplitudes{:});
%
% group_labels_widths = arrayfun(@(x) repmat({x}, size(trimmed_widths{strcmp({'OffOff', 'OffOn', 'OnOff', 'OnOn'}, x)})), {'OffOff', 'OffOn', 'OnOff', 'OnOn'}, 'UniformOutput', false);
% group_labels_widths = vertcat(group_labels_widths{:});
%
% group_labels_peakDists = arrayfun(@(x) repmat({x}, size(trimmed_peakDists{strcmp({'OffOff', 'OffOn', 'OnOff', 'OnOn'}, x)})), {'OffOff', 'OffOn', 'OnOff', 'OnOn'}, 'UniformOutput', false);
% group_labels_peakDists = vertcat(group_labels_peakDists{:});
...
function ANOVA_plot_allStates_v2(data, group_labels, measureName, yLabel, anovaTitle)

% % Ensure group_labels is a cell array of character vectors
% if ~iscellstr(group_labels) && iscell(group_labels)
%     % Convert cell array of strings to categorical directly
%     group_labels = categorical(group_labels);
% elseif isstring(group_labels)
%     % Convert string array to cell array of character vectors
%     group_labels = cellstr(group_labels);
%     group_labels = categorical(group_labels);
% else
%     error('Group labels must be a cell array of character vectors or a string array.');
% end

% % If group_labels is a string array, convert to cell array of character vectors
% if isstring(group_labels)
%     group_labels = cellstr(group_labels);
% end
%
% % Check that group_labels is a cell array of character vectors
% if ~iscellstr(group_labels)
%     error('Group labels must be a cell array of character vectors.');
% end
...
means = zeros(1, length(unique_groups));
stds = zeros(1, length(unique_groups));
