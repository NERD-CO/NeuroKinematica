function row_indices = plot_LFPbeta_byDepthTrial(LFPs_tbl, STN_depth, trialNum, repIdx, fs, lfpCol)
...
% Plot each requested rep (normalized PSD of beta band)
for rep_i = repList
    ...
    beta_zerophase_filt = filtfilt(beta_bandpass_filt, LFP_seg);

    % compute power spectral density (PSD) of LFP segment(s) - pspectrum function
    [beta_Pxx,beta_Fxx] = pspectrum(beta_zerophase_filt, fs,'FrequencyLimits',[0 50],'FrequencyResolution',3); % 2 or 3
    ...
    xlabel('Frequency (Hz)');
    ylabel('Power (dB)');
    title([plot_title ' | PSD of LFP beta (13-30 Hz)']);
