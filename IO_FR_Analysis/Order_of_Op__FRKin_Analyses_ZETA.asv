...
%% Notes re: getIFR: Compute instantaneous firing rate (IFR) + plot PSTHs
...
%% Single-Unit (spike clusters) - run_IFR_PSTH_byDepthMove
...
%% Spikes SU - 3D-PCA Convex Hull and Silhouette Plots

SaveDir = fullfile(FR_Kin_Dir, 'Aggregate Zeta Plots', '3D PCA Stats');

PCA_Silhouette_Stats = plot_ZETA_3DPCA_ConvexHull_byDepth(MasterZETA, ...
    'SaveDir', SaveDir, 'DoSilhouettePlot', true, 'FigPrefix', 'ZETA_PCA3D_Silhouette');


%% MUA - 3D-PCA Convex Hull and Silhouette Plots

PCA_Silhouette_Stats_MUA = plot_ZETA_3DPCA_ConvexHull_byDepth_MUA(MasterZETA_MUA, ...
    'SaveDir', SaveDir, 'DoSilhouettePlot', true, 'FigPrefix', 'MUA_ZETA_PCA3D_Silhouette');


%% Spikes SU - 3D-Feature Trajectories

SU_PCA_traj = plot_ZETA_TrajectoryAnalysis_SU(MasterZETA, ...
    'SaveDir', SaveDir, ...
    'FigPrefix', 'SU_ZETA_TrajFeat', ...
    'Nboot', 500, ...
    'DoProcrustes', true, ...
    'BootMinUnits', 5, ...
    'UseIntegralFeature', true);


%% MUA - 3D-Feature Trajectories

MUA_PCA_traj = plot_ZETA_TrajectoryAnalysis_MUA(MasterZETA_MUA, ...
    'SaveDir', SaveDir, ...
    'FigPrefix', 'MUA_ZETA_TrajFeat', ...
    'Nboot', 500, ...
    'DoProcrustes', true);


%% SU - Procrustes Distances

DistTbl_SU = vertcat( ...
    SU_PCA_traj.PerDepth(1).ProcDist, ...
    SU_PCA_traj.PerDepth(2).ProcDist, ...
    SU_PCA_traj.PerDepth(3).ProcDist);

plot_PCA_ProcDist_SummaryFigure_v2(DistTbl_SU, ...
    'SaveDir', SaveDir, ...
    'FigName', 'SU_ProcrustesDistance_Summary', ...
    'DoSave', true);

%% MUA - Procrustes Distances

DistTbl_MUA = vertcat( ...
    MUA_PCA_traj.PerDepth(1).ProcDist, ...
    MUA_PCA_traj.PerDepth(2).ProcDist, ...
    MUA_PCA_traj.PerDepth(3).ProcDist);

plot_PCA_ProcDist_SummaryFigure_v2(DistTbl_MUA, ...
    'SaveDir', SaveDir, ...
    'FigName', 'MUA_ProcrustesDistance_Summary', ...
    'DoSave', true);
