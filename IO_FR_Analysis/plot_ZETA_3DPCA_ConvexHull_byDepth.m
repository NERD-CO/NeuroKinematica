...
for d = 1:numel(depthOrder)
    ...
    ttl = sprintf('3D PCA | %s', depthNames(dz)); % 3D PCA + Hull projection
    ...
    % ----- Legend: use dummy scatters so legend colors are opaque -----
    mvOrder = string(U.ActiveMoveOrder);
    hLeg2 = gobjects(numel(mvOrder),1);
    for mIdx = 1:numel(mvOrder)
        mv = char(mvOrder(mIdx));
        c  = moveColorMap(mv);
        % dummy handle (doesn't plot any points)
        hLeg2(mIdx) = scatter3(ax3, NaN, NaN, NaN, ...
            60, c, 'filled', ...
            'MarkerFaceAlpha', 1.0, ...
    ...
    if U.DoSilhouettePlot
        ...
        if okSil
            ...
            title(axS, sprintf('Silhouette | %s', depthNames(dz)), 'Interpreter','none');
            ...
            for mIdx = 1:numel(mvOrder)
                ...
                silSummaryRows(end+1,:) = {depthNames(dz), mv, nnz(mm), ...
                    mean(silVals(mm),'omitnan'), ...
                    median(silVals(mm),'omitnan'), ...
                    std(silVals(mm),'omitnan')};
            end
        else
            title(axS, sprintf('Silhouette not computed (insufficient groups) | depth %s', depthNames(dz)), 'Interpreter','none');
        end


        % % --- Combined histogram of silhouette values, colored by MoveType ---
        % figure('Color','w'); axH1 = axes; hold(axH1,'on'); box(axH1,'on'); grid(axH1,'on');
        % mvOrder = string(U.ActiveMoveOrder);
        % mvPerUnit = string(mvPerUnit(:));
        % silVals = silVals(:);
        % % Common bin edges for all groups (so bars line up)
        % nBins = 15;
        ...
        % 
        %     c = moveColorMap(char(mv));  % color map (MoveType -> RGB)
        %     h(mIdx) = histogram(axH1, silVals(idx), ...
        %         'BinEdges', edges, ...
        %         'DisplayStyle','bar', ...
        %         'FaceColor', c, ...
        %         'FaceAlpha', 0.35, ...
        %         'EdgeColor', 'none');  % clean overlay
        % end
        % xlim(axH1, [-1 1]);
        % xlabel(axH1, 'Silhouette value');
        % ylabel(axH1, 'Count');
        % title(axH1, sprintf('Silhouette distribution by MoveType | %s', depthNames(dz)), 'Interpreter','none');
        % legend(axH1, cellstr(mvOrder), 'Location','northeast');
        % % overall median line
        % xline(axH1, median(silVals(isfinite(silVals))), '-k', 'median', 'LineWidth', 1.25);


        % % --- Stacked histogram of silhouette values, colored by MoveType ---
        ...
        % for mIdx = 1:numel(mvOrder)
        %     c = moveColorMap(char(mvOrder(mIdx))); % color map (MoveType -> RGB)
        %     b(mIdx).FaceColor = c;
        %     b(mIdx).EdgeColor = 'none';
        % end
        % xlim(axH2, [-1 1]);
        % xlabel(axH2, 'Silhouette value');
        % ylabel(axH2, 'Count');
        % title(axH2, sprintf('Silhouette (stacked) by MoveType | %s', depthNames(dz)), 'Interpreter','none');
        ...
        for i = 1:numel(cats)
            ...
            h = histogram(silVals(idx), 15);  % handle
...
x_wall = xmax + (0.2+mx); % flip --> YZ projection on BACK-right plane
