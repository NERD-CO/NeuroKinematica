...
Active_moveTypes = {'HAND OC','HAND PS','ARM EF'};  % consistent plotting order
...
if ~isAllDepths
    ...
    for mIdx = 1:numel(Active_mtOrder)
        mv = Active_mtOrder{mIdx};
        idx = mtPerUnit == mv;
        if ~any(idx), continue; end

        if isKey(Active_moveColorMap, mv)
    ...
    % Legend
    legend(ax, hLegend, Active_mtOrder, 'Location','northeastoutside', 'FontSize', 11);
    ...
    if ~isempty(U.SavePath)
        if ~exist(U.SavePath,'dir'), mkdir(U.SavePath); end
        outSingle = fullfile(U.SavePath, sprintf('ZETA_PCA_3D_depth_NoRest_%s.png', char(depthCodeSingle)));
        exportgraphics(hFig, outSingle, 'Resolution',300);
        fprintf('Saved single-depth 3D PCA figure to:\n  %s\n', outSingle);
    end
end
...
if U.DoTiled || isAllDepths
    ...
    hLeg = gobjects(numel(Active_mtOrder),1);
    for mIdx = 1:numel(Active_mtOrder)
        mv = Active_mtOrder{mIdx};
        c  = Active_moveColorMap(mv);
        % dummy invisible scatter for legend only
    ...
    lgd = legend(firstAx, hLeg, Active_mtOrder, 'Location','eastoutside');
    lgd.Title.String = 'MoveType';
    ...
    % Save tiled figure
    if ~isempty(U.SavePath)
        outTile = fullfile(U.SavePath, 'ZETA_PCA_3D_allDepths_NoRest.png');
        exportgraphics(hTile, outTile, 'Resolution',300);
        fprintf('Saved tiled 3-depth 3D PCA figure to:\n  %s\n', outTile);
    end
end
