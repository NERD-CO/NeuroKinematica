...
for SpkF = spikeFields
    ...
    for m = 1:numel(move_types)
        for d = 1:numel(U.DepthIDs)
            ...
            % --- Compute mean IFR in peri-movement window ---
...
%% Compute IFR_norm (within-unit z-score of IFR_mean_Hz across unit's conditions)

nRows = height(IFR_PSTH_Summary);
IFR_mean_Hz_col = IFR_PSTH_Summary.IFR_mean_Hz;

unitID = IFR_PSTH_Summary.SpikeField; % (C1, C1_MUA, etc.)
...
for g = 1:numel(unitLevels)
    ...
    if sum(~isnan(vals)) >= 2
        ...
        if sig > 0
            IFR_norm(idx) = (vals - mu) ./ sig;
        else
            IFR_norm(idx) = 0;  % all same → define as 0
        end
    else
        IFR_norm(idx) = 0;      % only one condition for this unit/electrode
    end
end
