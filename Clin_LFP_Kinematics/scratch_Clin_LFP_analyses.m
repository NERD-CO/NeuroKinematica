...
pspectrum(cleanLFP_s1,250,'FrequencyLimits',[0 50],'FrequencyResolution',3) % 2 or 3
[bPxx,bFxx] = pspectrum(cleanLFP_s1,250,'FrequencyLimits',[0 50],'FrequencyResolution',3); %
bPxxP = pow2db(bPxx);
...
%% Apply Filter, Hilbert, and PSD code above to all LFP streams
...
for i = 1:length(L_streamsofInt_OffMed)
    ...
    % Calculate instantaneous phase and freq. using Hilbert Transform
    % [inst_freq_filtered, ts_LFP_L] = HilbertTransform(cleanLFP_L, fs);
    hilbert_L = hilbert(cleanLFP_L);
    inst_phase_L = angle(hilbert_L);
    inst_freq_L = diff(unwrap(inst_phase_L))./(2*pi*diff(ts_LFP));

    % Filter instantaneous frequency for 13-30 Hz
    bandpass_filt = designfilt('bandpassiir','FilterOrder',4, ...
        'HalfPowerFrequency1',13,'HalfPowerFrequency2',30, ...
        'SampleRate',fs);
    inst_freq_filtered = filtfilt(bandpass_filt, inst_freq_L);
    inst_freq_filtered2 = [inst_freq_filtered, 0];

    % Plot instantaneous frequency for filtered LFP beta band
    figure;
    plot(ts_LFP, inst_freq_filtered2);
    xlim([0 round(max(ts_LFP))]);
    ylabel('Frequency (Hz)');
    xlabel('Time (s)');
    title(sprintf('Instantaneous Frequency of Filtered LFP Beta Band (13-30 Hz), LSTN Stream %d', i));

    % Compute power spectrum of LFP stream sampled at fs
    pspectrum(cleanLFP_L, 250, 'FrequencyLimits', [0 50],'FrequencyResolution',3); % 2 or 3
    [bPxx,bFxx] = pspectrum(cleanLFP_L,250,'FrequencyLimits',[0 50],'FrequencyResolution',3); %
    ...
    xlabel('Frequency (Hz)');
    ylabel('Power (dB)');
    title(sprintf('Power Spectral Density of LSTN LFP Stream %d', i));

    % % Plot the spectrogram
    % figure;
    % [s, f, t, p] = spectrogram(cleanLFP_L, 256, 250, 256, fs, 'yaxis');
    % surf(t, f, 10*log10(p), 'EdgeColor', 'none');
    % axis xy; axis tight; colormap(jet); view(0, 90);
    % xlabel('Time (s)');
    % ylabel('Frequency (Hz)');
    % title(sprintf('Spectrogram of LSTN LFP Stream %d', i));
    % colorbar;

    % Beta Bursting Analysis
    [beta_power_time_courses, beta_peak_frequency, bursts, time] = burstingAnalysis(lfp_data_L, fs);
    ...
    xlabel('Time (s)');
    ylabel('Frequency (Hz)');
    title(sprintf('Continuous Wavelet Transform of LSTN LFP Stream %d', i));
    ...
    xlabel('Time (s)');
    ylabel('Beta Power');
    title(sprintf('Beta Bursting Dynamics for LSTN LFP Stream %d', i));
    hold off;
end
...
for i = 1:length(R_streamsofInt_OffMed)
    ...
    lfp_data_R = R_streamsofInt_OffMed{i};

    % Target signal (LFP) sampling rate at 250 Hz (samples per sec)
    ts_LFP = 0:1/250:(height(lfp_data_R)-1)/250;

    % Filter ECG
    ecgClean_R = perceive_ecg(transpose(lfp_data_R),250,0);
    cleanLFP_R = ecgClean_R.cleandata;

    % Calculate instantaneous phase and freq. using Hilbert Transform
    % [inst_freq_filtered, ts_LFP_L] = HilbertTransform(cleanLFP_L, fs);
    hilbert_R = hilbert(cleanLFP_R);
    inst_phase_R = angle(hilbert_R);
    inst_freq_R = diff(unwrap(inst_phase_R))./(2*pi*diff(ts_LFP));

    % Filter instantaneous frequency for 13-30 Hz
    bandpass_filt = designfilt('bandpassiir','FilterOrder',4, ...
        'HalfPowerFrequency1',13,'HalfPowerFrequency2',30, ...
        'SampleRate',fs);
    inst_freq_filtered = filtfilt(bandpass_filt, inst_freq_R);
    inst_freq_filtered2 = [inst_freq_filtered, 0];
    ...
    ylabel('Frequency (Hz)');
    xlabel('Time (s)');
    title(sprintf('Instantaneous Frequency of Filtered LFP Beta Band (13-30 Hz), RSTN Stream %d', i));

    % Compute power spectrum of LFP stream sampled at fs
    pspectrum(cleanLFP_R, 250, 'FrequencyLimits', [0 50],'FrequencyResolution',3); % 2 or 3
    [bPxx,bFxx] = pspectrum(cleanLFP_R,250,'FrequencyLimits',[0 50],'FrequencyResolution',3); %
    bPxxP = pow2db(bPxx); % power to decibel conversion
    ...
    xlabel('Frequency (Hz)');
    ylabel('Power (dB)');
    title(sprintf('Power Spectral Density of RSTN LFP Stream %d', i));

    % % Plot the spectrogram
    % figure;
    % [s, f, t, p] = spectrogram(cleanLFP_R, 256, 250, 256, fs, 'yaxis');
    % surf(t, f, 10*log10(p), 'EdgeColor', 'none');
    % axis xy; axis tight; colormap(jet); view(0, 90);
    % xlabel('Time (s)');
    % ylabel('Frequency (Hz)');
    % title(sprintf('Spectrogram of RSTN LFP Stream %d', i));
    % colorbar;

    % Beta Bursting Analysis
    [beta_power_time_courses, beta_peak_frequency, bursts, time] = burstingAnalysis(lfp_data_R, fs);
    ...
    % Trim the LFP data
    lfp_data_R_trimmed = lfp_data_R(1:drop_off_index);

    % Continuous wavelet transform
    figure;
    cwt(lfp_data_R_trimmed, fs, 'FrequencyLimits', [1 125], 'VoicesPerOctave', 14);
    xlabel('Time (s)');
    ylabel('Frequency (Hz)');
    title(sprintf('Continuous Wavelet Transform of LSTN LFP Stream %d', i));
    ...
    xlabel('Time (s)');
    ylabel('Beta Power');
    title(sprintf('Beta Bursting Dynamics for RSTN LFP Stream %d', i));
    hold off;
end


%% stat comparison --> Jamovi

% calc. mean beta
% ANOVA across conditions (Jamovi)
% from matlab: need vector of hemispheres, mean beta, conditions
% accounts for mult. comp. correction (type 1 error: FP likelihood)
% 2-way ANOVA: hemisphere and condition

% from ANOVA comparisions that were sig dif,
% KS two-sample test to compare distrib across conditions (matlab)
% kstest2
% inputs: 2 PSDs being compared
% outputs: p-val, test statistic

% pairwise comparision
% p-val correction, apply Bonferroni factor (# of comparisions) to lower alpha
% False Discovery Rate (FDR): alternative way to correct p-val
...
%% test - LSTN Off Med, Off Stim - Isolate LFP and Video of iterest (LFP stream 3, DLC session 1)
...
streamOfInt_OffOff = L_streamOfInt_s1;
...
% Target signal (LFP) sampling rate at 250 Hz (samples per sec)
ts_LFP_OffOff = 0:1/250:(height(streamOfInt_OffOff)-1)/250;
...
%title('Kinematics and LFP: Off Med, Off Stim @ 0 mA')

ecg = perceive_ecg(transpose(streamOfInt_OffOff),250,0);
...
%% HOC segments (LFP stream 3, DLC session 1), LSTN Off Med, Off Stim

% Extract start and stop frame indices for 'Hand OC' (LFP stream 3, DLC session 1)
HOC_start_index_OffOff = eucINDICIES.StartInd(HandMov_indices(1));
HOC_stop_index_OffOff = eucINDICIES.StopInd(HandMov_indices(1));

[~ , startINDir] = min(abs(HOC_start_index_OffOff - trimFrame_int_OffOff));
[~ , endINDir] = min(abs(HOC_stop_index_OffOff - trimFrame_int_OffOff));
...
% Extract the data for Hand OC
HOC_LFP_OffOff = streamOfInt_OffOff(startINDir:endINDir);
HOC_dlc_trimmed_OffOff = dlc_trimmed_OffOff_int(HOC_start_index_OffOff:HOC_stop_index_OffOff, :);

% Create synchronized Kinematics and LFP plot foe HOC task (LFP stream 3, DLC session 1)
figure;
tiledlayout(2,1,"TileSpacing","tight")

% Plot Kinematics
fTip1_X_smooth = smoothdata(HOC_dlc_trimmed_OffOff.fTip1_x,'gaussian',70);
xTime_Kinematics = linspace(0, length(fTip1_X_smooth)/60, length(fTip1_X_smooth));
nexttile
plot(xTime_Kinematics, fTip1_X_smooth);
xlabel('Time (s)')
ylabel('fTip1, X deflection')
...
% Plot LFP
xTime_LFP = linspace(0, length(HOC_LFP_OffOff)/250, length(HOC_LFP_OffOff));
ecg = perceive_ecg(HOC_LFP_OffOff', 250, 0);
nexttile
...
% Target signal (LFP) sampling rate at 250 Hz (samples per sec)
ts_LFP_HOC_OffOff = 0:1/250:(height(HOC_LFP_OffOff)-1)/250;

ecgClean_HOC_OffOff = perceive_ecg(transpose(HOC_LFP_OffOff),250,0);

cleanLFP_HOC_OffOff = ecgClean_HOC_OffOff.cleandata;

% Calculate instantaneous phase and freq. using Hilbert transform
hilbert_HOC_OffOff = hilbert(cleanLFP_HOC_OffOff);
inst_phase = angle(hilbert_HOC_OffOff);
inst_freq = diff(unwrap(inst_phase))/(2*pi*t);
...
figure;
plot(ts_LFP_HOC_OffOff,inst_freq_filtered2);
xlim([0 round(max(ts_LFP_HOC_OffOff))])
ylabel('frequency (Hz)')
xlabel('time (seconds)')
title('Instantaneous Frequency of Filtered LFP Beta Band (13-30 Hz)');
...
% define 'lfp_data' as a LFP data matrix with dimensions [samples x trials]
lfp_data_HOC_OffOff = HOC_LFP_OffOff;
...
% 
% % continuous wavelet transform on trimmed LFP
% lfp_data_HOC_OffOff_trimmed = lfp_data_HOC_OffOff(1:drop_off_index);
% figure;
% cwt(lfp_data_HOC_OffOff_trimmed,fs,'FrequencyLimits',[1 125],'VoicesPerOctave',14); % try 14, mults of 7'
% xlabel('Time (s)');
% ylabel('Frequency (Hz)');
% title('Continuous Wavelet Transform');
% %colorbar;
...
%% test - LSTN Off Med, On Stim - Isolate LFP and Video of iterest (LFP stream 9, DLC session 7)
...
% Define LFP stream of interest:
streamOfInt_OffOn = L_streamOfInt_s7;
...
% Target signal (LFP) sampling rate at 250 Hz (samples per sec)
ts_LFP_OffOn = 0:1/250:(height(streamOfInt_OffOn)-1)/250;
...
title('Kinematics and LFP: Off Med, On Stim @ optimal mA')

ecg = perceive_ecg(transpose(streamOfInt_OffOn),250,0);
...
%% HOC segments (LFP stream 9, DLC session 7) - Off Med, On Stim
...
[~ , startINDir] = min(abs(HOC_start_index_OffOn - trimFrame_int_OffOn));
[~ , endINDir] = min(abs(HOC_stop_index_OffOn - trimFrame_int_OffOn));
...
% Extract the data for Hand OC
HOC_LFP_OffOn = streamOfInt_OffOn(startINDir:endINDir);
HOC_dlc_trimmed_OffOn = dlc_trimmed_OffOn_int(HOC_start_index_OffOn:HOC_stop_index_OffOn, :);

% Create synchronized Kinematics and LFP plot foe HOC task (Off Med, On Stim @ max mA)
figure;
tiledlayout(2,1,"TileSpacing","tight")

% Plot Kinematics
fTip1_X_smooth = smoothdata(HOC_dlc_trimmed_OffOn.fTip1_x,'gaussian',70);
xTime_Kinematics = linspace(0, length(fTip1_X_smooth)/60, length(fTip1_X_smooth));
nexttile
plot(xTime_Kinematics, fTip1_X_smooth);
xlabel('Time (s)')
...
% Plot LFP
xTime_LFP = linspace(0, length(HOC_LFP_OffOn)/250, length(HOC_LFP_OffOn));
ecg = perceive_ecg(HOC_LFP_OffOn', 250, 0);
nexttile
plot(xTime_LFP, ecg.cleandata);
xlabel('Time (s)')
ylabel('LFP (uV)')
