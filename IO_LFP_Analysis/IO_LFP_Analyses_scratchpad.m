...
% loop through all trials in depth
...
% viz raw vs. beta_zerophase signal 
plot(beta_zerophase_filt)
...
cwt_power_torr = zeros(length(f0), length(lfp_data_test));
...
for i = 1:length(f0)
    ...
    cwt_power_torr(i, :) = abs(conv(lfp_data_test, wavelet, 'same')).^2; % square to compute power
end

figure;
pcolor(cwt_power_torr)
shading flat
...
[cwt_power, cwt_freq] = cwt(lfp_data_test,fs,'FrequencyLimits',[1 125],'VoicesPerOctave',14); % try 14, mults of 7
figure;
...
% 1) find index of artifact, chop signal before that artifact,  run following
% burst detection code 
% 2) artifact removal methods exploration
