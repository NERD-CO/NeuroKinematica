...
for spk_mat_name = 1:length(SPKmatnames)
    ...
    % Add spike_ID and move_ID columns
...
standard_col_order = {'MoveN', 'MoveType', 'BeginF', 'EndF', 'spike_ID', 'move_ID'};
...
cd(ClustSpkTimesDir)
% writetable(All_SpikesPerMove_Tbl, 'All_SpikesPerMove_Tbl.csv');
...
temp_All_col = {};
% loop through each table in All_moveTble_array
for tbl_i = 1:length(All_spikesPermove)
    tbl_1 = All_spikesPermove{tbl_i};
    if isempty(tbl_1)
        continue
    ...
    % define column names in current table iteration within All_moveTbl_array
    col_names = tbl_1.Properties.VariableNames;
    missing_names = matches(col_names, temp_All_col); % identifies which column names in col_names are already present in temp_All_col.
    temp_All_col = [temp_All_col, col_names(~missing_names)]; % appends unique column names from col_names to temp_All_col
end
...
for tbl_i = 1:length(All_spikesPermove)
    ...
    % determine location of each column name in temp_All_col
    col_idx = matches(temp_All_col, col_names); % identifies the indices of columns in temp_All_col that match the columns in col_names
    %col_match = strcmp(col_names, temp_All_col)

    temp_data = cell(height(tbl_1), length(temp_All_col));
    if all(col_idx)
        new_idx = zeros(length(temp_All_col),1);
        for i = 1:length(temp_All_col)
            new_idx(i) = find(matches(col_names, temp_All_col{i}));
        end
         temp_data(:,new_idx) = table2cell(tbl_1);
    else
         temp_data(:,col_idx) = table2cell(tbl_1);
