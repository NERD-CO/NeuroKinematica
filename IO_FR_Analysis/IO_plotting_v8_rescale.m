function IO_plotting_v8(CaseDate, CaseDate_hem, ephys_offset)
...
for d = 1:3
    for m = 1:numel(movements)
        ...
        if isempty(trials)
            text(0.5,0.5,'No Data','HorizontalAlignment','center','Color',[0.7 0.7 0.7]);
            axis([0 0.45 0 1]); % minimal placeholder axis
            continue; 
        end
        ...
        N = numel(trials);
        scaleFactor = 0.5;
        maxY = ceil(N * scaleFactor); 
        
        % Compute FR stats for title
        FR_trials = cellfun(@(x) numel(x)/0.5, trials); % Hz (window length = 0.5s)
        mu_FR = mean(FR_trials); 
        sd_FR = std(FR_trials);
        ...
        xlim([-0.05 0.45]); ylim([0 maxY]);
        if d == 3, xlabel('Time (s)', 'FontSize', fontLabel); end
        if m == 1, ylabel(depth_labels{d}, 'FontSize', fontLabel); end
        title(sprintf('%s (%.1f ± %.1f Hz)', movements{m}, mu_FR, sd_FR), 'FontSize', fontTitle);
    end
end

sgtitle('Multi-Movement Raster (Rescaled)', 'FontSize', fontTitle, 'FontWeight', 'bold');
saveas(gcf, strrep(savePath, '.png', '_rescaled.png'));
savefig(strrep(savePath, '.png', '_rescaled.fig'));
close;
end
...
for d=1:3
    ...
    N_move = numel(trialsM);
    for t=1:N_move
        yScaled = t * scaleFactor;
        plot(trialsM{t}, yScaled*ones(size(trialsM{t})), '.', 'Color', colors(d,:), 'MarkerSize',8);
    end
    ylim([0 max(1, ceil(N_move * scaleFactor))]); xlim([-0.05 0.45]);
    if d==3, xlabel('Time (s)','FontSize',fontLabel); end
    ...
    if ~isempty(trialsM) && ~isempty(trialsR)
        ...
        text(0.15, max(1, ceil(N_move * scaleFactor))+0.5, sprintf('\\mu = %.1f \\pm %.1f Hz', mu, sd), 'FontSize', 10, 'Color', colors(d,:));
        
        [~,p] = ttest2(FR_move, FR_rest);
        pVals(d) = p; cohenD_vals(d) = computeCohensD(FR_move, FR_rest);
        stars = getSigStars(p);
        text(0.3, max(1, ceil(N_move * scaleFactor))+0.5, stars, 'FontSize', 14, 'FontWeight', 'bold', 'Color', colors(d,:));
    end
end
