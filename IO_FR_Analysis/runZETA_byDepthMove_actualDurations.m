...
for SpkF = spikeFields
    ...
    for moveT_i = 1:numel(move_types)
        for depth_i = 1:numel(ParsedInputs.DepthIDs)
            ...
            % % ---- Build stitched inputs with true [start stop] ---
            ...
            %     'EndField',    ParsedInputs.EndField, ...
            ...
            if strcmp(mv, 'REST')
                ...
                [spkT, evTimes, useMaxDur, trialDur, kept_tbl] = makeZetaInputs_fromAOStartTimes_REST( ...
                    ...
                    'StartField',  ParsedInputs.StartField, ...
                    'EndField',    ParsedInputs.EndField, ...
                    'StartOffset_s', 0.150, ... % 250 ms
                    'MinDur_s',      0.750, ... % 750 ms
                    'MaxDur_s',      1.000, ... % 1000 ms
                    'PreWindow_s',   0.000, ... % 0 ms
                    'PostWindow_s',  ParsedInputs.PostWindow_s, ... % 0 ms
                ...
                [spkT, evTimes, useMaxDur, trialDur, kept_tbl] = makeZetaInputs_fromAOStartStopTimes( ...
                    ...
                    'StartField',   ParsedInputs.StartField, ...
                    'EndField',     ParsedInputs.EndField, ...
                    'PreWindow_s',  ParsedInputs.PreWindow_s, ...  % 50 ms
                    'PostWindow_s', ParsedInputs.PostWindow_s, ... % 0  ms
            ...
            % 1) If EndField is causing error (durations <= 0), try a common fallback name
            ...
            if hadError
                ...
                rows(end+1,:) = { ...
                    curSF, mv, dz, height(kept_tbl), useMaxDur, ...         % SpikeField, MoveType, Depth, nTrials, UseMaxDur_s
                    ...
                    [], useMaxDur, []};                                     % VecSpikeT, ZetaUseMaxDur, VecLatencies
            ...
            rows(end+1,:) = { ...
                ...
                MeanD, MeanZ, MeanP, ...
                sZETA.vecSpikeT(:)', ...            % timestamps of spike times (corresponding to vecD) at ZETA sampling grid
