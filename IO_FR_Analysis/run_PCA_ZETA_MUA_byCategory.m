...
%
% Do PCA on MUA ZETA temporal deviation vectors (vecD_MUA) for each
% MoveType × STN depth, using vecTime_MUA as the time axis.
%
% Treat each subject/hemisphere MUA (per MoveType×Depth) as one "unit".
% Plots PC1 per MoveType for each Depth (3 rows: t/c/b).

fprintf('[OK] MasterZETA_MUA: %d total entries\n', height(MasterZETA_MUA));

% Required columns
reqVars = {'MoveType','Depth','vecD_MUA','vecTime_MUA','PrettyLabel'};
...
hasT = ~cellfun(@isempty, MasterZETA_MUA.vecTime_MUA);
M = MasterZETA_MUA(hasD & hasT, :);
if isempty(M)
    error('No rows with non-empty vecD_MUA and vecTime_MUA.');
end

fprintf('[OK] Unique MoveTypes (MUA): %s\n', strjoin(unique(M.MoveType,'stable'), ', '));
...
for d = 1:numel(depths)
    ...
    for m = 1:numel(moveTypes)
        ...
        if height(catRows) < 2
            continue;   % need >=2 units for meaningful PCA
        end

        % Reference time axis from first row
        t_ref = catRows.vecTime_MUA{1};
        if isempty(t_ref) || ~isnumeric(t_ref)
            warning('Empty/non-numeric vecTime_MUA for %s × %s; skipping.', mv, dz);
            continue;
        end
        t_ref = t_ref(:);
